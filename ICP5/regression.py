...
train = pd.read_csv('winequality-red.csv') # data set
data = train.select_dtypes(include=[np.number]).interpolate().dropna() #handling missing value
# Top 3 correlation
data_correlation = data.corr()['quality'][:11]
sorted_data = data_correlation.sort_values(ascending=False)
print("Descending order",sorted_data[0:3])
...
model = lr.fit(features_train, target_train) # Train the model using the training sets
